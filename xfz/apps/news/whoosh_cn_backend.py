...
from haystack.backends import BaseEngine, BaseSearchBackend, BaseSearchQuery, EmptyResults, \
    log_query
...
from whoosh.fields import BOOLEAN, DATETIME, IDLIST, KEYWORD, NGRAM, NGRAMWORDS, NUMERIC, Schema, \
    TEXT
...
class WhooshSearchBackend(BaseSearchBackend):
    ...
    def build_schema(self, fields):
        ...
        for field_name, field_class in fields.items():
            if field_class.is_multivalued:
                ...
                schema_fields[field_class.index_fieldname] = NUMERIC(stored=field_class.stored,
                                                                     numtype=int,
                                                                     field_boost=field_class.boost)
                ...
                schema_fields[field_class.index_fieldname] = NGRAM(minsize=3, maxsize=15,
                                                                   stored=field_class.stored,
                                                                   field_boost=field_class.boost)
            elif field_class.field_type == 'edge_ngram':
                schema_fields[field_class.index_fieldname] = NGRAMWORDS(minsize=2, maxsize=15,
                                                                        at='start',
                                                                        stored=field_class.stored,
                                                                        field_boost=field_class.boost)
            else:
    ...
    def search(self, query_string, sort_by=None, start_offset=0, end_offset=None,
        ...
        if limit_to_registered_models is None:
            limit_to_registered_models = getattr(settings, 'HAYSTACK_LIMIT_TO_REGISTERED_MODELS',
                                                 True)
    ...
    def _process_results(self, raw_page, highlight=False, query_string='', spelling_query=None,
        ...
        for doc_offset, raw_result in enumerate(raw_page):
            ...
            if model and model in indexed_models:
                ...
                result = result_class(app_label, model_name, raw_result[DJANGO_ID], score,
                                      **additional_fields)
